...
const getInitialState = ({
  isLoading,
  layout
}: SkeletonContentProps): State => ({
  isLoading,
  layout
});

const getDefaultProps = (): SkeletonContentProps => ({
...
> {
  ...
  getGradientStartDirection = (): Direction => {
  ...
  getGradientEndDirection = (): Direction => {
  ...
  static getDerivedStateFromProps(
    nextProps: SkeletonContentProps,
    prevState: State
  ) {
    if (
  ...
  getPositionRange = (boneLayout: CustomViewStyle): Array<number> => {
    let outputRange: Array<number> = [];
  ...
  getBones = (
    layout: Array<CustomViewStyle>,
    children: any
  ): Array<JSX.Element> => {
    if (layout.length > 0) {
      const iterator: Array<number> = new Array(layout.length);
  ...
  renderLayout = (
    isLoading: boolean,
    bones: Array<JSX.Element>,
    children: any
  ): Array<JSX.Element> => (isLoading ? bones : children);
